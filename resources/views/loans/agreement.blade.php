...
<br><br>
   ...
   <div class="devanagari header"> साप्ताहिक किश्त भुगतान अनुबंध/शपथ पत्र</div>
    ...
    मै {{ $loan->customer->name }}  पुत्र/पुत्री/पत्नी {{ explode(',', $loan->customer->address)[0] ?? 'N/A' }}
     निवासी  {{ $loan->customer->address }}  आज दिनांक {{ \Carbon\Carbon::parse($loan->sale_date)->format('d-m-Y') }} को राव एनर्जी सॉल्यूशंस से 
     {{ $loan->finance_period_weeks/4 }} बैटरी क्रय कर रहा/रही हूँ जिसकी कुल कीमत ₹ {{ number_format($loan->billed_amount,2) }} है 
     जिसमें से ₹ {{ number_format($loan->total_payment,2) }} का भुगतान कर रहा/रही हूँ और बकाया ₹ {{ number_format($loan->financed_amount,2) }} 
     का भुगतान मै अगले {{ $loan->finance_period_weeks }} सप्ताह में कर दूंगा/दूंगी।
    </p>

    <p class="devanagari-text">मैं प्रत्येक सप्ताह के {{ $loan->weekly_installment_day }} के दिन अपना साप्ताहिक किश्त जो कि ₹ 
        {{ number_format($loan->weekly_installment_amount,2) }} है शाम 6 बजे तक किश्त की पूरी रकम जमा कर दूंगा/दूंगी| 
        अगर मैं {{ $loan->weekly_installment_day }} के दिन शाम 6 बजे तक साप्ताहिक किश्त की पूरी रकम नही जमा करता/करती हूँ 
    ...
    <p class="devanagari-text">मैं अगर {{ $loan->finance_period_weeks/4 }} साप्ताहिक किश्त नहीं जमा करता/करती हूँ तो 
        ...
        <p class="devanagari-text">मैं अगर {{ $loan->finance_period_weeks/2 }}  सप्ताह में कुल बकाया धनराशि का आधा नहीं जमा करता/करती हूँ तो मैं क्रय की हुई बैटरियां राव एनर्जी सॉल्यूशंस को वापस लेने की स्वैच्छिक अनुमति देता/देती हूँ एवं किसी भी प्रकार के कोई भुगतान एवं अपने द्वारा दी गयी पुराने बैटरी की वापसी का दावा नहीं करूंगा/करूंगी।</p>
    
    <p class="devanagari-text">मैं अगर {{ $loan->finance_period_weeks }} सप्ताह में कुल बकाया धनराशि नहीं जमा करता/करती हूँ तो मैं क्रय की हुई बैटरियां 
    ...
    <p class="devanagari-text">अगर  {{ $loan->finance_period_weeks }}  सप्ताह में कुल बकाया धनराशि ( जिसमें non payment penalty और late payment penalty शामिल है ) मैं नहीं अदा कर पाता/पाती हूँ और राव एनर्जी सॉल्यूशंस क्रय की हुईं बैटरियां वापस नहीं लेते है तो मैं बकाया धनराशि ( जिसमें non payment penalty और late payment penalty शामिल है ) पर 1% late payment penalty के रूप में प्रत्येक सप्ताह के अंत में बकाया धनराशि में बढ़ाने की अनुमति राव एनर्जी सॉल्यूशंस को देता/देती हूँ।</p>
    <br><br>
    <p class="devanagari-text">दिनांक: {{ \Carbon\Carbon::parse($loan->sale_date)->format('d-m-Y') }} </p> 
